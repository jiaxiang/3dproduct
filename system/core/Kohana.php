...
final class Kohana {
	...
	public static function auto_load($class)
	{
		...
		{
			$imports = self::$configuration['core']['imports'];
			if (!empty($imports))
			{
				$clsname = trim($class, ' _');
				$clsname = strtolower($clsname);

				foreach ((array)$imports as $import)
				{
					$filename = rtrim($import, ' /') . '/' . str_replace('_', '/', $clsname) . '.php';
					if (file_exists($filename))
					{
						require $filename;

						if (!class_exists($clsname, FALSE) AND !interface_exists($clsname, FALSE))
						{
							return FALSE;
						}
