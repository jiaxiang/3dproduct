...
class Cache_Core {
    ...
    static public function factory($id = NULL)
    {
        ...
        if (!isset(self::$instances[$id]))
        {
            if (self::isExists($id)) {
                ...
                //self::$instances[$id] = new Memcache;
                //self::$instances[$id]->connect('127.0.0.1', 11211);
            }
        }
    ...
    static public function get($key)
    {
        ...
        $type = ($position = strpos($key, '.')) > 0 ? substr($key, 0, $position) : $key;
        ...
        if (!self::isDenied($type)) {
            try{
                ...
                throw new MyRuntimeException($ex->getMessage(), $ex->getCode());
            }
    ...
    static public function isDenied($type)
    {
        ...
        return isset(self::$config['denied'][$type]) && self::$config['denied'][$type];
    }
