<?php
class Secoder_cn_Core{
	public static $seKey = 'jingbo365.com';
	...
	public static $bg = array(250, 250, 250);  // 背景
	...
	public static function entry() {
		...
		$w = self::$imageL;
		$h = self::$imageH;
		$fontface = dirname(__FILE__) . '/ttfs/bygfz3500.ttf';
		...
		//var_dump(mb_substr($str,6,1));die();
		$code = "";
		for($i = 0; $i < $ch_num; $i++) {
			$r_min = 0;
			$r_max = mb_strlen($str) - 1;
			$j = mt_rand($r_min, $r_max);
			$code .= mb_substr($str, $j, 1);
		}
		//var_dump($code);die();
		//$_SESSION['code']=$code; //赋值给SESSION
		
		self::$_image=imagecreatetruecolor($w,$h);
		$bkcolor=imagecolorallocate(self::$_image, self::$bg[0], self::$bg[1], self::$bg[2]);
		imagefill(self::$_image,0,0,$bkcolor);
		if (self::$useNoise == true) {
			for($i=0;$i<15;$i++){
				$fontcolor=imagecolorallocate(self::$_image,mt_rand(0,255),mt_rand(0,255),mt_rand(0,255));
				imagearc(self::$_image,mt_rand(-10,$w),mt_rand(-10,$h),mt_rand(30,300),mt_rand(20,200),55,44,$fontcolor);
			}
			for($i=0;$i<255;$i++){
				$fontcolor=imagecolorallocate(self::$_image,mt_rand(0,255),mt_rand(0,255),mt_rand(0,255));
				imagesetpixel(self::$_image,mt_rand(0,$w),mt_rand(0,$h),$fontcolor);
			}
		}
		$sess_code = array();
		for ($i = 0; $i < $ch_num; $i++) {
			$fontcolor = imagecolorallocate(self::$_image,mt_rand(0,120),mt_rand(0,120),mt_rand(0,120));
			$codex = mb_substr($code, $i, 1);
			$sess_code[$i] = $codex;
			imagettftext(self::$_image, $fontsize, mt_rand(-10,10),40*$i+25,mt_rand(40,50), $fontcolor,$fontface,$codex);
		}
		Session::instance()->set(Session::instance()->id().self::$seKey.".code",implode('',$sess_code));// 把校验码保存到session
		Session::instance()->set(Session::instance()->id().self::$seKey.".time",time());// 验证码创建时间
		//$fontcolor = imagecolorallocate($im, 0, 0, 0);
		//imagettftext($im, 15, 0, 10, 20, $fontcolor,$fontface,$code);
	...
	public static function check($code) {
		// 验证码不能为空
		$session_code = Session::instance()->get(Session::instance()->id().self::$seKey.".code");
	
		if(empty($code) || empty($session_code))
		{
			return false;
		}
		// session 过期
		if(time() - Session::instance()->get(Session::instance()->id().self::$seKey.".time") > self::$expire)
		{
			Session::instance()->delete(Session::instance()->id().self::$seKey.".code");
			Session::instance()->delete(Session::instance()->id().self::$seKey.".time");
			return false;
		}
