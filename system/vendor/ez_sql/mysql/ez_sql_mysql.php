	...
	class ezSQL_mysql extends ezSQLcore
	{
		...
		*/
	    ...
	    function reconnect(){
            ...
            $return_val = false;
            if ( ! $this->dbh = @mysql_connect($this->dbhost,$this->dbuser,$this->dbpassword, true) )
            {
                $this->register_error($ezsql_mysql_str[2].' in '.__FILE__.' on line '.__LINE__);
                $this->show_errors ? trigger_error($ezsql_mysql_str[2],E_USER_WARNING) : null;
            }else{
		...
		*/
		...
		function escape($str)
		{
			...
			is_resource($this->dbh) or $this->reconnect();
		    if(!$this->dbh )
            {
...
//            $version_main = floatval($version[0].'.'.$version[1]);
//            if($version_main <4.1) return;
