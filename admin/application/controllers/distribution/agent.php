...
class Agent_Controller extends Template_Controller 
{
	...
	public function add($userId)
	{
		...
		do {
			...
			$duplicate = $this->agentDao->get_by_invite_code($aAgent['invite_code']);
		}while ($duplicate != null);
		
		if($this->agentDao->agent_exist($aAgent))
		{
			remind::set(Kohana::lang('o_agent.agent_has_exist'),request::referrer(),'error');
		}
		if($this->agentDao->add($aAgent))
		{
			remind::set(Kohana::lang('o_global.add_success'),'distribution/agent', 'success');
		}
		else 
		{
