...
class Doc_category_Controller extends Template_Controller {

	public function __construct()
	{
		parent::__construct();
        role::check('doc_category');
	}
	...
	public function add()
	{
		...
		if($_POST)
		{
			...
			{
				remind::set(Kohana::lang('o_site.doc_category_cannot_null'),'site/doc_category/add');
			}
			else 
			{
				if(Mydoc_category::instance()->name_exist($category_name))
				{
					remind::set(Kohana::lang('o_site.doc_category_has_exist'));
				}
                else
                {
    				if(Mydoc_category::instance()->add($_POST))
    				{
    					remind::set(Kohana::lang('o_global.add_success'),'site/doc_category','success');
    				}
    				else
    				{
		...
		$this->template->content = new View("site/doc_category_add");
