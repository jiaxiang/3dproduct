...
<!-- header_content(end) -->
<!--** content start**-->
<form id="add_form" name="add_form" method="post" action="<?php echo url::base();?>site/delivery/do_edit">
  <input id="delivery_id" name="delivery_id" value="<?php echo $data['id']; ?>" type="hidden"/>
  <div class="out_box"> 
      ...
      <table cellspacing="0" class="table_overflow">
        ...
        <col />
          ...
          <tr>
            ...
            <td id="country_control"><input type="radio" name="type" value="0" <?php if ($data['type'] == 0) : ?> checked<?php endif; ?>/> 统一设置         &nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="type" value="1" <?php if ($data['type'] == 1) : ?> checked<?php endif; ?>/> 指定配送国家和费用 </td>
          </tr>
    ...
    <div id="set_country_price" class="new_order_con fixfloat " style="display:<?php if($data['type'] == 0) :?>none<?php endif;?>">
      <table cellspacing="0" class="table_overflow">
      <col width="150">
        <col />
         <tr>
  ...
  <div id="country_sel_ifm" class="ui-dialog-content ui-widget-content" style="height:270px;min-height:100px;width:auto;">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		//国家费用类型选择
		$('#country_control').live('click', function(e){
			var o = $(e.target);
			if (typeof o.attr('type') != 'undefined' && o.attr('type').toUpperCase() == 'RADIO') {
				...
				if (v == 1) {
					...
					$('#set_country_price').find(':input').removeAttr('disabled');
					...
					$('#set_country_price').find(':input').removeClass('required').attr('disabled', 'true');
	    ...
	    var country_sections = function(){
			...
			var t = {
				...
				add: function(sec) {
					...
					if(typeof sec != 'undefined' && typeof sec['country_ids'] != 'undefined'){
						var ps = selected_country_ids[idx];
						id = sec['country_ids'].split('-');
						for(var data in id){
							if (typeof ps[id[data]] == 'undefined') {
								ps[id[data]] = true;
							} 
						}
						selected_country_ids[idx] = ps;
					}				
					...
					row_city.append('<span class="small_btn right"><span id="del_country_' + idx + '" class="del"></span></span>');	
					row_city.append('配送地区 <input type="text" id="country_names_' + idx + '" name="country_names_' + idx + '" class="input_300px text required" value="' + (typeof sec != 'undefined' && typeof sec['country_names'] != 'undefined' ? sec['country_names'] : '') + '" readonly="true" style="background-color:#f1f1f1"/>');
					row_city.append('<span id="sel_country_' + idx + '" class="sel_country"><img src="'+url_base+'images/icon_view.gif"></img></span>');	
					...
					row_city.append('<input type="hidden" name="country_ids_' + idx + '" id="country_ids_' + idx + '"  value="' + (typeof sec != 'undefined' && typeof sec['country_ids'] != 'undefined' ? sec['country_ids'] : '') + '" style="width:10px;"><input type="hidden" name="des_indexs[]" value="' + idx + '" style="width:10px;">');
					...
					row_exp.append('<input type="hidden" name="countries_use_exp_' + idx + '" value="' + (typeof sec != 'undefined' && typeof sec['use_exp'] != 'undefined' ? sec['use_exp'] : '0') + '"></input> <span class="delivery_msg"  value="1" onclick="javascript:changeExp(this);"><a>取消公式</a></span>');
					...
					if (typeof sections[idx] != 'undefined') {
						if (t.count() > 1) {
		                    ...
		                    if ($('input[name="ids_' + k + '"]').length > 0) {
		                        ...
		                        var url = url_base + 'site/delivery/del_country?ids=' + ids + '&delivery_id=' + delivery_id;
		                        $.ajax({
		                            url: url,
		                            type: 'GET',
		                            dataType: 'json',
        ...
        //加载存在的国家信息
    	<?php if (isset($country_area) && !empty($country_area)) : ?>
			country_sections.load(<?php echo json_encode($country_area); ?>);
			label = true;
		<?php endif; ?>
	});
