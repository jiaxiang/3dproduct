...
<div style="margin-top: 0px; border-top: 0px none;" class="division" id="product_attributes_btn_box">
    <input type="button"<?php if (!empty($product['goods'])) : ?> style="display:none;"<?php endif; ?> value=" 开启规格 " class="ui-button ui-widget ui-state-default ui-corner-all ui-state-focus" id="ptype_configurable_open" role="button" aria-disabled="false">
    <input type="button"<?php if (empty($product['goods'])) : ?> style="display:none;"<?php endif; ?> value=" 添加规格项 " class="ui-button ui-widget ui-state-default ui-corner-all" id="ptype_configurable_att" role="button" aria-disabled="false">
    <input type="button"<?php if (empty($product['goods'])) : ?> style="display:none;"<?php endif; ?> value=" 关闭规格 " class="ui-button ui-widget ui-state-default ui-corner-all"  id="ptype_configurable_close" role="button" aria-disabled="false">
    <input type="button"<?php if (empty($product['goods'])) : ?> style="display:none;"<?php endif; ?> value=" 添加货品 " class="ui-button ui-widget ui-state-default ui-corner-all" id="ptype_configurable_agd" role="button" aria-disabled="false">
</div>
	...
	var ptype_simple = function() {
        ...
        var setBtns = function() {
        	if (typeof t != 'undefined' && t.getGoodCount() > 0) {
        		$('#ptype_configurable_close').show();
        		$('#ptype_configurable_att').show();
        		$('#ptype_configurable_agd').show();
                $('#attributes_box').show();
        		$('#ptype_configurable_open').hide();
                $('#product_good_default_info_box').hide();
        	} else {
        		$('#ptype_configurable_close').hide();
        		$('#ptype_configurable_att').hide();
        		$('#ptype_configurable_agd').hide();
                $('#attributes_box').hide();
        		$('#ptype_configurable_open').show();
		...
		var crtNewGood = function() {
			...
			for (var aid in attributes) {
				...
				row += required?'<span name="pdt_attr_set_option" style="cursor:pointer;">':'<span>';
    				...
    				if (typeof attributes[aid] == 'undefined') {
    					...
    					for (var i = cs.length - 1; i >= 0; i --) {
    						...
    						if (cs[i].nodeName.toUpperCase() == 'TH') {
                                ...
                                h += required?'<span name="pdt_attr_set_option" style="cursor:pointer;">':'<span>';
    	...
    	$(document).ready(function(){
	         ...
	         * 开启规格按钮绑定事件
	         */
	        $('#ptype_configurable_open').bind('click', function(){
	        	t.show();
	        });
	         ...
	         * 添加规格按钮绑定事件
	         */
	        $('#ptype_configurable_att').bind('click', function(){
	        	t.show();
	        });
	         ...
	         * 关闭规格按钮绑定事件
	         */
	        $('#ptype_configurable_close').bind('click', function(){
	         ...
	         * 添加货品按钮绑定事件
	         */
	        $('#ptype_configurable_agd').bind('click', crtNewGood);
