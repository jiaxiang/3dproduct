        ...
        <div>
                <div class="public_right public">
                    <form id="search_form" name="search_form" method="GET" action="<?php echo url::base() . url::current();?>">
                        <input name="collection_id" type="hidden" value="<?php echo $collection_id; ?>"/>
            ...
            <form id="product_relations" action="/product/collection/put_products" method="post">
            <input id="collection_id" name="collection_id" type="hidden" value="<?php echo $collection_id; ?>"/>
            <div class="division">
	...
	$(document).ready(function(){
		...
		$('#add_product_relation').live('click', function(){
			...
			if(typeof ps != 'defined' && $.count(ps) > 0){
	    		...
	    		url = url_base + 'product/collection/put_products?collection_id=' + collection_id + '&relation_ids=' + relation_ids;
	    		location.href = url;
        	}else{
        		parent.showMessage('操作失败', '<font color="#990000">请选择所要关联的商品！</font>');
        	}			
		});
