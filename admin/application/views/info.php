...
<div class="new_content">
    <div class="newgrid">
		...
		<div class="new_skip_info">
        	<p class="skip_suc"><?php echo html::specialchars($render_struct['msg']); ?></p>
			<p>如果您不做出选择，将在<?php echo $render_struct['action']['time'];?>秒后跳转到第一个链接地址。</p>
			<ul>
            	<li><a href="javascript:<?php if ($render_struct['action']['type'] == 'location') : ?>location.href='<?php echo $render_struct['action']['url']; ?>';<?php else : ?>history.go(-1);<?php endif; ?>">上一页</a></li>
				<?php if (!empty($render_struct['action']['links'])) : ?>
				<?php foreach ($render_struct['action']['links'] as $name => $link) : ?>
				<li><a href="<?php echo $link; ?>"><?php echo htmlspecialchars($name); ?></a></a></li>
				<?php endforeach; ?>
				<?php endif; ?>
            </ul>
        </div>       
