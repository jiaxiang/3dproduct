...
class Mysettlemonthrptdtl_Core extends My
{
	//对象名称(表名)
    protected $object_name = 'ag_settle_month_dtl';
	...
	{
		return ORM::factory('ag_settle_month_dtl')->count_all();
	}
	...
	public function agent_exist($data)
	{
		...
		$count = ORM::factory('ag_settle_month_dtl')->where($where)->count_all();
	...
	public function get_by_user_id($user_id)
	{
		...
		$user = ORM::factory('ag_settle_month_dtl')->where($where)->find();
		if($user->loaded){
	...
	{
		$list = array();
		$orm_instance = ORM::factory('ag_settle_month_dtl')
    	        ->select('uuu.lastname as clientlastname, agent.lastname as agentlastname, ag_settle_month_dtls.*')
    	        ->join('users as agent', 'agent.id', 'ag_settle_month_dtls.agentid', 'LEFT')
    	        ->join('users as uuu', 'uuu.id', 'ag_settle_month_dtls.user_id', 'LEFT');
    	// 处理输入条件
		$where = array();
		$in = array();
