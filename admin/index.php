...
//    $is_win && $project_root .= 'D:';
    ...
    function f($var, $die = '')
    {
    	...
    	file_put_contents('f.txt',var_export($var, TRUE));
    ...
    /**
     * 生产环境绝对地址配置 zhu
     * for production project root path in linux system
     */
    $project_root = '/zzshop';
}
...
define('FRONTPATH', get_front_path());
define('WEBROOT', FRONTPATH);
function get_front_path()
{
    $arr = explode('/', SYSPATH);
    array_pop($arr);
    array_pop($arr);
    return implode('/', $arr).'/front/';
}
...
// Clean up
unset($kohana_application, $kohana_modules, $kohana_system, $kohana_attachment);
