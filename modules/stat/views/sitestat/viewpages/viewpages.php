<div class="new_content">
    ...
    <div class="pro_ie6">
        ...
        <div class="contentin new_pro_con fixfloat">
			<div class="count_con">
                <h2>受访页面（<?php echo $data['date_from']; ?> —— <?php echo $data['date_to']; ?>）</h2>
                <div class="tips">
                    <span class="fB">小贴士：</span> 	受访页面分析为您提供网站任意时间内各页面被访问的情况。
                </div> 
                <div class="date"><script type="text/javascript" src="/js/year_month_date.js"></script>
                	<span style="float:left">
                		<a href="/sitestat/viewpages" <?php if(isset($istoday)) echo "class=\"current\""; ?>>今日</a> | 
                		<a href="/sitestat/viewpages/yesterday" <?php if(isset($isyesterday)) echo "class=\"current\""; ?>>昨日</a> | 
                		<a href="/sitestat/viewpages/thismonth" <?php if(isset($isthismonth)) echo "class=\"current\""; ?>>本月</a> | 
                		<a href="/sitestat/viewpages/recent30days" <?php if(isset($isrecent30days)) echo "class=\"current\""; ?>>最近30天</a> 
                		[<a href="/sitestat/viewpages/oneday/<?php echo date('Y-m-d',strtotime( $data['date_from'] )-86400) ?>">前一天</a>] 
                		[<a href="/sitestat/viewpages/oneday/<?php echo date('Y-m-d',strtotime( $data['date_to'] )+86400) ?>">后一天</a>]
                		&nbsp;&nbsp;&nbsp;&nbsp;
                	</span>
					<form method="POST" action="/sitestat/viewpages/fewdays">
                ...
                <div class="tab">
                    <div class="tab_title_box fixfloat">
                        <ul>
                            <span class="left fB f14px">受访页面占比</span>
                        </ul><br>
                        <?php
                        if ($data['flash1'] == 'none') {
                        	echo '您查询的日期暂时没有受访页面的数据！';
                        }else {
                        	echo $data['flash1'];
                        }
                        ?>
                    ...
                    <div class="tab_content_box" style="border:1px solid #98AED0;">
						...
						<tr>
						<th colspan="5">
							<span class="left fB f14px">受访页面列表（从<?php echo $data['date_from']; ?>到<?php echo $data['date_to']; ?>）</span>
							<div class="right">
						...
						<tr class="headings">
						<th>受访页面</th><th>来访次数</th><th>独立IP</th><th>人均浏览次数</th>
						</tr>
						<?php
						for ($i=0; $i<count($data['view_pages']); $i++){
							
							echo "
							<tr>
							<td>{$data['view_pages'][$i]['url']}</td>
							<td>{$data['view_pages'][$i]['pv']}</td>
							<td>{$data['view_pages'][$i]['ip']}</td>
							<td>{$data['view_pages'][$i]['pv_ip']}</td>
							</tr>
							";
						}
						?>
