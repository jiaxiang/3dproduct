 ...
 * 
 */
class Fromsite_Controller extends Template_Controller {
	...
	public function index(){
        ...
        try {
			...
			$content = new View($this->package_name.'/'.$this->class_name.'/'.'fromsite');
			...
			$this->template->content->sitestat_left->fromsite = 1;
	...
	private function getpicsrc($data){
		...
		for ($i=0; $i<count($data); $i++){
			...
			$pts .= ($pts == '') ? $data[$i]['site'] : ','.$data[$i]['site'];
			$chart_data .= "{$data[$i]['site']};{$data[$i]['pv']}\n";
		}
		$chart_data = urlencode($chart_data);
		$src1 = "/sitestat/chart?type=pc&w=1000&h=400&ps=$ps&pts=$pts";
	...
	private function manage_data($data, $date_from, $date_to){
		...
		for ($i=0; $i<count($sites); $i++){
			$sites[$i]['ip']        = $sites[$i]['ip_count'];
			$sites[$i]['pv_ip']     = $sites[$i]['ip']==0 ? 0 : substr($sites[$i]['pv']/$sites[$i]['ip'],0,5);
			$sites[$i]['jump_rate'] = substr( $sites[$i]['jump_count']/$sites[$i]['pv']*100, 0, 5).'%';
		}
		$array['sites'] = $sites;
