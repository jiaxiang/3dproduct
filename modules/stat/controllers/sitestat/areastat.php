 ...
 * 
 */
class Areastat_Controller extends Template_Controller {
	...
	/**
	 * phprpc服务器地址，定义在config/phprpc.php文件$config['remote']['statking']['host']中
	 *
	 * @var string
	 */
	...
	public function index(){
        ...
        try {
			...
			$content = new View($this->package_name.'/'.$this->class_name.'/'.'area');
			...
			$this->template->content->sitestat_left->areastat = 1;
	...
	private function get_request_data( $date_from, $date_to, $order_by=0 ){
        ...
        $page = $this->getpage();
        //发送请求获取原始数据
		$data_all = $client->get_data_country_by_time_range($statking_id, $time_from, $time_to, $order_by, $page, $this->perpage, $sign);
	...
	private function getpicsrc($data){
		...
		for ($i=0; $i<count($data); $i++){
			...
			$pts .= ($pts == '') ? $data[$i]['name'] : ','.$data[$i]['name'];
			$chart_data .= "{$data[$i]['name']};{$data[$i]['pv']}\n";
		}
	...
	private function manage_data($data, $date_from, $date_to){
		$array = array();
		$array['total_pv'] = $data['total']['pv'];
		$array['total_ip'] = $data['total']['ip_count'];
		$array['total_viewtime'] = $data['total']['viewtime'];
		$array['total_jump_rate'] = $data['total']['pv'] == 0 ? '0%' : substr( $data['total']['jump_count']/$data['total']['pv']*100, 0, 5).'%';
		...
		for ($i=0; $i<count($areas); $i++){
			$areas[$i]['ip']        = $areas[$i]['ip_count'];
			$areas[$i]['pv_ip']     = $areas[$i]['ip']==0 ? 0 : substr($areas[$i]['pv']/$areas[$i]['ip'],0,5);
			$areas[$i]['jump_rate'] = substr( $areas[$i]['jump_count']/$areas[$i]['pv']*100, 0, 5).'%';
		}
		$array['areas'] = $areas;
