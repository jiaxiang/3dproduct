...
fh_asp_utf8
  ...
  │  │
  │  ├alipay_notify.asp┈┈┈┈┈┈┈┈┈┈支付宝通知处理类文件（闲置）
  │  │
...
※注意※
需要配置的文件是：alipay_config.asp、sendgoods.asp

index.asp仅是支付宝提供的付款入口模板文件，可选择使用。
如果商户网站根据业务需求不需要使用，请把sendgoods.asp作为与商户网站网站相衔接页面。
如果需要使用index.asp，那么sendgoods.asp文件无需更改，只需配置好alipay_config.asp文件
...
功能：把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串，并对其做URL编码转换
